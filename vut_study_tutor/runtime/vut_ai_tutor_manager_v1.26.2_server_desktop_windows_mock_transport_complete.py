...
RELEASE_VERSION = "1.26.2"
RELEASE_MARKER = "VUT-AI-TUTOR-1.26.2-SERVER-DESKTOP-WINDOWS-MOCK-TRANSPORT-COMPLETE"
...
def parse_desktop_config_roots(explicit: str = "") -> list[Path]:
    ...
    if appdata:
        parent = Path(appdata)
        for name in ("Open WebUI", "open-webui", "open-webui-desktop"):
...
def ensure_backend(args: argparse.Namespace, report: dict[str, Any], *, operation: str) -> str:
    """Resolve a live backend; TutorOnly may start the standalone Desktop automatically."""
    if args.target == "Desktop" and args.mode == "TutorOnly":
...
def self_test(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "desktop_backend_resolution": "launch-relative-timeout-bounded-probe-v2",
